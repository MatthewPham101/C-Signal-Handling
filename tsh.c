...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv)) {
        // Not a built-in command, fork a child process
...
int builtin_cmd(char **argv) 
{
    if (strcmp(argv[0], "quit") == 0) {
        ...
        listjobs(jobs);
        return 1;  // return 1 Command was built-in
...
void do_bgfg(char **argv) 
{
    ...
    // Check if a valid JID or PID is provided
    if (argv[1] == NULL) {
    ...
    // Send the appropriate signal whether it is a fg or bg
    if (strcmp(argv[0], "bg") == 0) {
        if (job->state == ST) {
            // If the job is stopped, resume it in the background
...
void waitfg(pid_t pid)
{
    ...
    while (1) {
        ...
        if (job == NULL || job->state != FG) {
            // If the job is not found or not in the foreground, return
            return;
        }
...
void sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        // Reap terminated or stopped child processes
        ...
        if (WIFEXITED(status)) {
            ...
            // If the child terminated due to an uncaught signal
            // this will check if there is a job if not return;
            ...
            char temp[100];
            char msg[100] = "";
            sprintf(temp, "Job [%d] (%d) terminated by signal %d\n", job->jid, pid, WTERMSIG(status));
            strcat(msg, temp);
            ...
            if (job != NULL) {
                ...
                char temp[100];
                char msg[100] = "Job";
                sprintf(temp, " [%d] (%d) stopped by signal %d\n", job->jid, pid, WSTOPSIG(status));
                strcat(msg, temp);
                write(1, msg, sizeof(msg) - 1);
                fflush(stdout);
...
void sigint_handler(int sig) 
{
    ...
    if (fg_pid != 0) {
        ...
        if (job != NULL) {
            ...
            // Delete the terminated job
         
            char temp[100];
            char msg[100] = "";
            sprintf(temp, "Job [%d] (%d) terminated by signal %d\n", job->jid, fg_pid, SIGINT);
            strcat(msg, temp);
            write(1, msg, sizeof(msg) - 1);
